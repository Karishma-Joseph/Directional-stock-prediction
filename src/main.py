...
def generate_all_models():
    ...
    #          crossover_count, Metrics.EPS, Metrics.REVENUE, Metrics.PE, Metrics.DEBT_EQUITY, Metrics.PRICE_TO_BOOK, 'Score', 'Frequency', 'Sentiment score']
    x_col = [Metrics.RSI, Metrics.MACD_DECISION, Metrics.OBV, col_derivative, col_distance,
             crossover_count, 'Score', 'Frequency', 'Sentiment score']
    ...
    #     else:
    #         model_type = ModelAttributes.INCREASE_DECREASE_CLASSIFICATION.format(
    #             "apple" + "_" + str(Metrics.FIFTEEN_MIN) + "_sentiment")
    #     generate_models(data=data, x_col=x_col, y_col=y_col, model_type=model_type)


def generate_stock_models(x_col, y_cols, company, semantic=True):
    files = os.listdir("data/training_data/")
    for y_col in y_cols:
        for file in files:
            if company in file:
